...
class GET_LABEL(): # get the label from files to feed into the model
    ...
    def __call__(self,index,dataframe,dic,label_folder):
        ...
        for i,j in enumerate(names):
            if j != 'NAN':
                if self.rotate:
                    img = Image.open('{0}\{1}\{2}'.format(dic,label_folder,j)).rotate(-90,expand = True)
                else:
...
def predict(image,transform_deeplab,transform_deeplab_detail,dlab2,dlab2_detail,label,original_size,eps_value = 'auto',thres=0.001,scale = 0.05,return_prob = True):

    device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
    
    validation = transform_deeplab(image).unsqueeze(0).to(device)
    try:
        coordinates,prob = get_region(dlab2(validation,interpolate = False).squeeze(),
                                      label = label,original_size = original_size,eps_value = eps_value,
                                      thres = thres,scale = scale)
    except:
        coordinates,prob = get_region(dlab2(validation).squeeze(),
                                      label = label,original_size = original_size,eps_value = eps_value,
                                      thres = thres,scale = scale)
    ...
    if coordinates!= None:
        ...
        for i,co in enumerate(coordinates):
            ...
            if 0 not in np.array(image)[int(y1):int(y2),int(x1):int(x2)].shape:
                ...
                if return_prob:
                    om = scipy.special.softmax(res_test.squeeze().detach().cpu().numpy(),axis = 0)[1,:,:]
                    om_list.append(om)
                else:
...
def get_region(density,label,original_size,eps_value = 'auto',scale = 0.1,thres = 0.001): # reversely get bounding boxes with corresponding probabilities from segmented results
    coor_list = []
    prob_list = []
    feature_map = torch.argmax(density, dim=0)
    feature_map[feature_map!=label] = 0
    feature_map[feature_map == label] = 1
    if len(torch.unique(feature_map))==1:
        return None,None
    else:
        ...
        if eps_value == 'auto':
            e = np.sqrt(feature_map_array.shape[0]*feature_map_array.shape[1])/20
        else:
            e = eps_value
        clustering = DBSCAN(eps=e).fit(np.c_[index[0],index[1]]) 
...
def find_window(img,dlab,transform_deeplab,device,scale = 0.1,max_count = 45, # automatically find the window
    ...
    coordinates,prob = get_region(dlab(validation,interpolate = False).squeeze(),
...
def draw_bounding_box(image,model,transform,colors,labelnames,dpi,fontsize = 14,line_width = 2,transparency = 0.5): # drawing bounding boxes for each class in the image
    ...
    text_com = []
    for i in labels:
        coordinates,prob = get_region(density,i,image.size,scale = 0.05)
